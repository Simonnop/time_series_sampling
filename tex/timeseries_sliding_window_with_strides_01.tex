...
\begin{tikzpicture}[scale=.5, every node/.style={minimum size=1cm}, on grid]
    ...
    \draw[draw=base03, fill=gray!70, thick] (17,2) rectangle (18,3);
        ...
        % X 窗口高亮（绿色，对应 X_i）
        \draw[base03, ultra thick, fill=green!30, fill opacity=0.6] (2, 0) rectangle (6, 3);
        \draw[step=1, base03, thick] (2, 0) grid (6, 3);
        % 保存 X 窗口四角坐标，用于与输出层连线
        \coordinate (XWBL) at (2, 0);
        \coordinate (XWBR) at (6, 0);
        \coordinate (XWTL) at (2, 3);
        \coordinate (XWTR) at (6, 3);
        % Y 窗口高亮（黄色，对应 Y_i）
        \draw[base03, ultra thick, fill=yellow!40, fill opacity=0.6] (6, 0) rectangle (9, 3);
        \draw[step=1, base03, thick] (6, 0) grid (9, 3);
        % 保存 Y 窗口四角坐标，用于与输出层连线
        \coordinate (YWBL) at (6, 0);
        \coordinate (YWBR) at (9, 0);
        \coordinate (YWTL) at (6, 3);
        \coordinate (YWTR) at (9, 3);
    ...
    \begin{scope}[xshift=2cm, yshift=6cm,
        ...
        \node[left=4pt, yshift=-4px, fill=white, inner sep=1.5pt]  at (0.0, 1.5) {$ \mathbf{x}_{2} \in \mathbb{R}^{L\times D} $};
        \node[right=4pt, yshift=4px, fill=white, inner sep=1.5pt] at (7, 1.5) {$ \mathbf{y}_{2} \in \mathbb{R}^{H\times D} $};
    \end{scope}
    % 画布右下角的用户自定义文字
