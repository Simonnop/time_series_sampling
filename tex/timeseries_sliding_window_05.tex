...
\begin{tikzpicture}[scale=.5, every node/.style={minimum size=1cm}, on grid]
    ...
    \draw[draw=base03, fill=gray!70, thick] (17,2) rectangle (18,3);
        ...
        % X 窗口高亮（绿色，对应 X_i）
        \draw[base03, ultra thick, fill=green!30, fill opacity=0.6] (5, 0) rectangle (9, 3);
        \draw[step=1, base03, thick] (5, 0) grid (9, 3);
        % 保存 X 窗口四角坐标，用于与输出层连线
        \coordinate (XWBL) at (5, 0);
        \coordinate (XWBR) at (9, 0);
        \coordinate (XWTL) at (5, 3);
        \coordinate (XWTR) at (9, 3);
        % Y 窗口高亮（黄色，对应 Y_i）
        \draw[base03, ultra thick, fill=yellow!40, fill opacity=0.6] (9, 0) rectangle (12, 3);
        \draw[step=1, base03, thick] (9, 0) grid (12, 3);
        % 保存 Y 窗口四角坐标，用于与输出层连线
        \coordinate (YWBL) at (9, 0);
        \coordinate (YWBR) at (12, 0);
        \coordinate (YWTL) at (9, 3);
        \coordinate (YWTR) at (12, 3);
