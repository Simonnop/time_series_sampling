...
\begin{tikzpicture}[scale=.5, every node/.style={minimum size=1cm}, on grid]
    ...
    \draw[draw=base03, fill=gray!70, thick] (8,2) rectangle (9,3);
        ...
        % X 窗口高亮（绿色，对应 X_i）
        \draw[base03, ultra thick, fill=green!30, fill opacity=0.6] (3, 0) rectangle (6, 3);
        ...
        % 选取某一时刻的向量 v_t（D 维）
        \draw[->, very thick, base03] (4.5, 0.0) -- (4.5, -0.4);
        \node[right=4pt] at (4.8, -0.6) {$ \mathbf{v}_t \in \mathbb{R}^{D} $};
    \end{scope}
    ...
    \begin{scope}[xshift=0, yshift=5cm,
        ...
        % 标签：每一帧标注 X_i 和 Y_i
        \node[above=4pt] at (1.5, 3.0) {$ \mathbf{x}_{4} \in \mathbb{R}^{T\times D} $};
        \node[above=4pt] at (4.5, 3.0) {$ \mathbf{y}_{4} \in \mathbb{R}^{H\times D} $};
    \end{scope}
\end{tikzpicture}
\end{document}
