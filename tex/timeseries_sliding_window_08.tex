...
\begin{tikzpicture}[scale=.5, every node/.style={minimum size=1cm}, on grid]
    ...
    \begin{scope}[xshift=2cm, yshift=6cm,
        ...
        \node[left=4pt, yshift=-4px, fill=white, inner sep=1.5pt]  at (0.0, 1.5) {$ \mathbf{x}_{9} \in \mathbb{R}^{L\times D} $};
        \node[right=4pt, yshift=4px, fill=white, inner sep=1.5pt] at (7, 1.5) {$ \mathbf{y}_{9} \in \mathbb{R}^{H\times D} $};
    \end{scope}
    % 画布右下角的用户自定义文字
