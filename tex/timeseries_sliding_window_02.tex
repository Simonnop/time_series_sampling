...
\begin{tikzpicture}[scale=.5, every node/.style={minimum size=1cm}, on grid]
    ...
    \draw[draw=base03, fill=gray!70, thick] (8,2) rectangle (9,3);
        ...
        % X 窗口高亮（绿色，对应 X_i）
        \draw[base03, ultra thick, fill=green!30, fill opacity=0.6] (2, 0) rectangle (5, 3);
        \draw[step=1, base03, thick] (2, 0) grid (5, 3);
        % 保存 X 窗口四角坐标，用于与输出层连线
        \coordinate (XWBL) at (2, 0);
        \coordinate (XWBR) at (5, 0);
        \coordinate (XWTL) at (2, 3);
        \coordinate (XWTR) at (5, 3);
        % Y 窗口高亮（黄色，对应 Y_i）
        \draw[base03, ultra thick, fill=yellow!40, fill opacity=0.6] (5, 0) rectangle (8, 3);
        \draw[step=1, base03, thick] (5, 0) grid (8, 3);
        % 保存 Y 窗口四角坐标，用于与输出层连线
        \coordinate (YWBL) at (5, 0);
        \coordinate (YWBR) at (8, 0);
        \coordinate (YWTL) at (5, 3);
        \coordinate (YWTR) at (8, 3);
        ...
        % 选取某一时刻的向量 v_t（D 维）
        \draw[->, very thick, base03] (3.5, 0.0) -- (3.5, -0.4);
        \node[right=4pt] at (3.8, -0.6) {$ \mathbf{v}_t \in \mathbb{R}^{D} $};
    \end{scope}
    ...
    \begin{scope}[xshift=0, yshift=5cm,
        ...
        % 标签：每一帧标注 X_i 和 Y_i
        \node[above=4pt] at (1.5, 3.0) {$ \mathbf{x}_{3} \in \mathbb{R}^{T\times D} $};
        \node[above=4pt] at (4.5, 3.0) {$ \mathbf{y}_{3} \in \mathbb{R}^{H\times D} $};
    \end{scope}
\end{tikzpicture}
\end{document}
